...
const APCNonTeachingHubPage = () => {
  const navigate = useNavigate();
  return (
    <div className="page-container">
      <button onClick={() => navigate('/apc')}
      ...
      <div style={{ marginBottom: '2rem' }}>
        <h1 style={{ fontSize: '1.375rem', fontWeight: 800, color: 'var(--text-primary)', marginBottom: '0.3rem' }}>Non-Teaching Staff</h1>
        <p style={{ color: 'var(--text-muted)', fontSize: '0.825rem' }}>Select a category to review staff appraisals and submit recommendations.</p>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.25rem' }}>
        <HubCard title="Junior Non-Teaching Staff" subtitle="Administrative, support, and technical staff at junior grades eligible for review."
          icon={<LuUsers size={24} />} color="#f59e0b" onClick={() => navigate('/apc/non-teaching/junior')} />
        <HubCard title="Senior Non-Teaching Staff" subtitle="Senior administrative, managerial, and professional staff eligible for review."
          icon={<LuBriefcase size={24} />} color="#8b5cf6" onClick={() => navigate('/apc/non-teaching/senior')} />
      </div>
    </div>
  );
};

export default APCNonTeachingHubPage;
